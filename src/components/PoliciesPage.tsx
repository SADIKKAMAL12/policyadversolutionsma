...
export default function PoliciesPage({ onAgree, language }: PoliciesPageProps) {
  ...
  return (
    <div ref={scrollContainerRef} className="space-y-6 p-6 lg:p-10">
      <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900 p-8 text-white shadow-[0_30px_80px_rgba(2,6,23,0.4)] backdrop-blur">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h1 className="relative mt-2 text-3xl font-bold tracking-tight text-transparent lg:text-4xl">
                ...
                <span
                  className="pointer-events-none absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent opacity-60"
            ...
            <div className="flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 text-sm font-semibold text-white/80 shadow-inner">
        ...
        <div className="pointer-events-none absolute inset-x-8 bottom-4 h-px bg-gradient-to-r from-transparent via-white/60 to-transparent" aria-hidden />
      </div>
        ...
        <div className="relative">
          <div ref={policyScrollRef} className="min-h-[60vh] max-h-[80vh] overflow-y-scroll">
            <div className="px-4 py-6 pr-8 sm:pl-8 sm:py-8 sm:pr-16">
              <div className="sticky top-0 z-10 -mx-4 -mt-6 border-b border-white/70 bg-gradient-to-b from-white/95 to-white/80 px-4 pb-6 pt-6 shadow-[0_20px_50px_rgba(15,23,42,0.08)] backdrop-blur sm:-mx-8 sm:px-8 dark:border-slate-900/70 dark:from-slate-950/95 dark:to-slate-950/70">
                <div className="flex flex-col gap-6">
                  ...
                  <div className="h-1.5 w-full overflow-hidden rounded-full bg-slate-100/80 dark:bg-slate-900/40">
                    <div
                      className="h-full rounded-full bg-gradient-to-r from-brand via-sky-500 to-indigo-500 transition-all"
                      style={{ width: `${Math.max(scrollProgress, 0.04) * 100}%` }}
                    />
                  </div>
                </div>
              </div>
              <div className="pt-10">
                {!hasScrolledToBottom && (
                  <div className="rounded-2xl border border-dashed border-slate-200/70 bg-white/90 p-4 text-sm text-slate-600 shadow-inner backdrop-blur dark:border-slate-800/60 dark:bg-slate-900/60 dark:text-slate-200">
          ...
          {!hasScrolledToBottom && isScrollable && (
            <div className="pointer-events-none absolute inset-x-0 bottom-6 flex justify-center">
              <button
                type="button"
                onClick={handleScrollPromptClick}
                className="pointer-events-auto inline-flex items-center gap-3 rounded-full border border-white/10 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-800 px-6 py-3 text-sm font-semibold text-white shadow-xl shadow-slate-900/40 backdrop-blur transition hover:-translate-y-0.5 dark:border-white/20 dark:bg-white/10"
